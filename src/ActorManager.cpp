...
namespace hdt
{
	...
	void ActorManager::Skeleton::attachArmor([[maybe_unused]] RE::NiNode* armorModel, RE::NiAVObject* attachedNode)
	{
		...
		if (!isFirstPersonSkeleton(skeleton.get()))
		{
			...
			auto system = SkyrimSystemCreator().createOrUpdateSystem(getNpcNode(skeleton.get()), attachedNode, &armor.physicsFile, std::move(armor.renameMap), nullptr);
	...
	void ActorManager::Skeleton::reloadMeshes()
	{
		for (auto& i : armors)
		{
			...
			if (!isFirstPersonSkeleton(skeleton.get()))
			{
				auto system = SkyrimSystemCreator().createOrUpdateSystem(npc.get(), i.armorWorn.get(), &i.physicsFile, std::move(i.renameMap), nullptr);
	...
	void ActorManager::Skeleton::scanHead()
	{
		...
		for (auto& headPart : this->head.headParts)
		{
			...
			auto system = SkyrimSystemCreator().createOrUpdateSystem(npc.get(), this->head.headNode.get(), &headPart.physicsFile, std::move(this->head.renameMap), nullptr);
